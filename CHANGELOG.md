...
- Keep Workbench signed in when an individual gateway launch or status refresh
  ...
  prevents a fresh `admin@100yen.org` sign-in from bouncing out before visual
  acceptance can run.
- Make the issue #100 Home task launcher visibly actionable: the “Start Work
  ...
  Browser, Approvals, and hosted Creative Studio when the signed-in role has
  access, while disabled rows explain that sign-in is required.
