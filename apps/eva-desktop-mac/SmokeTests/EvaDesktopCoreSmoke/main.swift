...
let olderManifest = WorkbenchReleaseManifest(version: "0.1.0", build: "1", downloadURL: trustedDownload)
let newerManifest = WorkbenchReleaseManifest(version: "0.1.1", build: "1", downloadURL: trustedDownload)
let newerBuildManifest = WorkbenchReleaseManifest(version: "0.1.0", build: "2", downloadURL: trustedDownload)
...
precondition(!WorkbenchUpdateClient.isTrustedUpdateURL(URL(string: "https://example.com/evaOS-Workbench-Beta-0.1.0.zip")!))
try WorkbenchUpdateClient.validate(WorkbenchReleaseManifest(version: "0.1.0", build: "1", downloadURL: trustedDownload, sha256: String(repeating: "a", count: 64), releaseNotesURL: URL(string: "https://www.electricsheephq.com/evaos-workbench")!))
