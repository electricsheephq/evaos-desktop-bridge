...
precondition(creativeStudioDesignGateDoc.contains("Disabled customers can hide the feature flag"))
...
precondition(releaseScriptSource.contains("Recent launches in Session Center"))
precondition(releaseScriptSource.contains("minting a fresh broker URL"))
precondition(releaseScriptSource.contains("Creative Studio wiring and Codex visible-GUI readiness polish"))

let trustedDownload = URL(string: "https://github.com/electricsheephq/evaos-workbench-releases/releases/download/evaos-workbench-v0.6.22/evaOS-Workbench-0.6.22.zip")!
let olderManifest = WorkbenchReleaseManifest(version: "0.6.21", build: "61", downloadURL: trustedDownload)
let newerManifest = WorkbenchReleaseManifest(version: "0.6.22", build: "1", downloadURL: trustedDownload)
let newerBuildManifest = WorkbenchReleaseManifest(version: "0.6.22", build: "63", downloadURL: trustedDownload)
...
try WorkbenchUpdateClient.validate(WorkbenchReleaseManifest(version: "0.6.22", build: "62", downloadURL: trustedDownload, sha256: String(repeating: "a", count: 64), releaseNotesURL: URL(string: "https://www.electricsheephq.com/evaos-workbench")!))
