...
precondition(releaseScriptSource.contains("durable helper mouse-action audit record before dispatching"))
precondition(releaseScriptSource.contains("separate completion or failure entry"))
precondition(releaseScriptSource.contains("authenticated dumb hands"))

let trustedDownload = URL(string: "https://github.com/electricsheephq/evaos-workbench-releases/releases/download/evaos-workbench-v0.6.17/evaOS-Workbench-0.6.17.zip")!
let olderManifest = WorkbenchReleaseManifest(version: "0.6.16", build: "56", downloadURL: trustedDownload)
let newerManifest = WorkbenchReleaseManifest(version: "0.6.17", build: "1", downloadURL: trustedDownload)
let newerBuildManifest = WorkbenchReleaseManifest(version: "0.6.17", build: "58", downloadURL: trustedDownload)
...
try WorkbenchUpdateClient.validate(WorkbenchReleaseManifest(version: "0.6.17", build: "57", downloadURL: trustedDownload, sha256: String(repeating: "a", count: 64), releaseNotesURL: URL(string: "https://www.electricsheephq.com/evaos-workbench")!))
