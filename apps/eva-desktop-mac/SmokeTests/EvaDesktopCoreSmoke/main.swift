...
precondition(workbenchModelSource.contains("try await openProviderAuthHandoff(response.connectURL)"))
...
let trustedDownload = URL(string: "https://github.com/electricsheephq/evaos-workbench-releases/releases/download/evaos-workbench-v0.6.4/evaOS-Workbench-0.6.4.zip")!
let olderManifest = WorkbenchReleaseManifest(version: "0.1.3", build: "1", downloadURL: trustedDownload)
let newerManifest = WorkbenchReleaseManifest(version: "0.6.5", build: "1", downloadURL: trustedDownload)
let newerBuildManifest = WorkbenchReleaseManifest(version: "0.6.4", build: "45", downloadURL: trustedDownload)
...
try WorkbenchUpdateClient.validate(WorkbenchReleaseManifest(version: "0.6.4", build: "44", downloadURL: trustedDownload, sha256: String(repeating: "a", count: 64), releaseNotesURL: URL(string: "https://www.electricsheephq.com/evaos-workbench")!))
