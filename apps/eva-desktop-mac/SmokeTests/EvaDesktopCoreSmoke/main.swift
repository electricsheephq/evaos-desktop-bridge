...
let newerManifest = WorkbenchReleaseManifest(version: "0.5.2", build: "1", downloadURL: trustedDownload)
let newerBuildManifest = WorkbenchReleaseManifest(version: "0.5.1", build: "32", downloadURL: trustedDownload)
...
try WorkbenchUpdateClient.validate(WorkbenchReleaseManifest(version: "0.5.1", build: "31", downloadURL: trustedDownload, sha256: String(repeating: "a", count: 64), releaseNotesURL: URL(string: "https://www.electricsheephq.com/evaos-workbench")!))
