...
def test_release_package_bundles_matching_bridge_helper() -> None:
    ...
    assert "Makes Home the default Workbench front door" in script
    assert "Connected Apps" in script
    assert "Business Browser" in script
    assert "hosted Comfy Cloud" in script
