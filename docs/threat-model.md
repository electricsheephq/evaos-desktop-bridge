...
- Expose a local computer-use helper IPC seam for health checks and narrow
  per-process mouse actions only after sender authorization and bridge-side
  policy, approval, kill-switch, target-process, and audit checks have already
  passed.
...
| Unauthenticated helper socket | The computer-use helper seam uses length-prefixed JSON, a short default `/tmp` Unix-socket path for macOS path limits, a rotated per-launch token file, peer-uid match, unsafe token-file rejection, regular-file socket path refusal, accepted-connection read timeouts, oversized-frame rejection before JSON parsing, and an exact command allowlist. Current live routing is limited to helper-owned per-process `mouse_action` click/scroll/drag via `CGEventPostToPid` plus fixed semantic `ax_action` verbs for fresh snapshot elements after bridge-side policy, approval, target-sensitive-app, target-process, and kill-switch checks. Browser web-content targets are treated as inert/escalated instead of successful. AX value writes are limited to editable native text roles and values are passed through 0600 temp files plus hashes, not long-lived argv/audit text. The bridge writes append-only `helper.mouse_action` / `helper.ax_action` authorized-dispatch audit records before IPC, passes that audit id through the helper request, and writes completion/failure records after the helper returns. No shell, AppleScript, Python, raw AX primitive passthrough, generic computer-use, iPhone action, or Codex mutation command is accepted. |
