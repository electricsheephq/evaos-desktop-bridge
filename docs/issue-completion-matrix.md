...
| `#242` Assigned Agent Model | One agent can be assigned to one user with allowed apps, allowed surfaces, budget, approval policy, kill switch state, owner/admin pause-revoke authority, and Today/Home status. Agent-only users must not see technical dashboards, Terminal, billing, or unassigned provider grants. | `WorkbenchAgentAssignment` decodes `evaos.agent_assignment.v1`, derives a first assignment from safe capability-manifest summaries, keeps provider-grant checks explicit, maps agent-only visibility to assigned surfaces plus the assigned-agent workspace, and publishes assigned-agent Home records through `WorkbenchMissionCardDeriver.assignedAgentCards`. Swift smoke covers the schema, grant denial, role policy, running/done/blocked status mapping, and non-broker resume route. |
| `#243` Home/Today V1 | Home should render business-readable Today items for app connection, approval, browser login, assigned-agent status, Company Brain source, and recent work; raw runtime/provider/grant/audit details should be collapsed under technical detail; supported cards should route to the correct Workbench/Broker surface. | `WorkbenchTodayItem` implements `evaos.today_item.v1`, `WorkbenchTodayItemDeriver` maps connected app, queue approval, Business Browser, assigned-agent, Company Brain, and recent broker evidence into business next actions, and `SessionCenterView` now renders a `Today` card grid with `Technical details` collapsed by default. Supported actions route to Connected Apps, Needs Your Okay, Business Browser, dashboard Company Brain in Business Browser, or brokered runtime resume. Swift smoke covers schema decode/rejection, human-language titles, provenance details, approval/company-brain/recent-work derivation, Company Brain handoff source wiring, and assigned-agent runtime route preservation. |
| `#244` AionUi Reference Spike | Native prototype covers agent/team cards, task launchers, assistant catalog source labels, provider/MCP readability, and pending permission badges. No Electron shell, YOLO defaults, local secret storage, renderer-visible keys, or bundled subprocess trust is adopted. Spike includes screenshot evidence and explicit keep/reject decisions. | `SessionCenterView` now includes a native `AionReferenceSpikeSection` that uses existing assignment, provider, capability-summary, Today, and approval evidence to render an Agent Workspace Preview. The ADR records AionUi keep/reject decisions and screenshot-evidence expectations. Swift smoke verifies the native section, forbidden Aion/Electron patterns, no package dependency adoption, and the ADR decision language. |
| `#245` ClickClack Chat Binding Spike | ClickClack is represented only as a wrapped Team Chat substrate: one evaOS account maps to one ClickClack workspace; one human, service bot, assigned agent, channel, and DM are represented; bot tokens are secret refs and revocable; embed does not require magic-link copy/paste; ClickClack does not own billing, membership, provider grants, agent assignment, or permission policy. | Workbench now includes a default-off brokered `Team Chat` runtime guarded by `VITE_EVAOS_TEAM_CHAT`, a versioned `evaos.team_chat_binding.v1` Core model, agent-only `team_chat` surface gating, ADR contract details, and Swift smoke coverage for runtime/status encoding, feature defaults, no raw bot tokens, and no ClickClack authority over evaOS account policy. |
