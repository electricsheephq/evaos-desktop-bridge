...
| `#101` Creative Studio hosted/configured ComfyUI design gate | ADR chooses hosted/configured URL first; customer journey covers login, persistence, configured URL, unavailable state, and support recovery; API lane describes grants, revocation, job submission, polling, output retrieval, and disabled states; VM-local path is explicitly deferred with graduation criteria. | `docs/creative-studio-hosted-comfyui-design-gate.md` records the hosted/configured ComfyUI-first ADR, Workbench customer journey, API grant lane, disabled/unconfigured customer behavior, and VM-local graduation criteria. `RuntimeKey.creativeStudio` remains feature-flagged, brokered, and non-bundled in the macOS app; issue `#102` remains the implementation epic for the real broker/proxy runtime and live ComfyUI canary. |
| `#102` Creative Studio / Hosted ComfyUI-first first implementation slice | Creative Studio becomes a real broker/proxy runtime for enabled customers without bundling ComfyUI, GPU workers, model storage, or workflow automation. Disabled/unconfigured customers see a clean unavailable state. | In progress: Workbench models `creative_studio` as a brokered runtime and the dashboard page resolves customer runtime registry state before calling the fixed `creative_studio` proxy session path. Full closeout still requires a live support VM/friendly customer ComfyUI canary and route proof. |
...
| Issue | Acceptance | Implemented |
| --- | --- | --- |
| `#97` Shared Browser first-class Workbench gateway | Shared Browser exposes broker runtime status and user-facing open/reconnect/reload/status controls without raw proxy errors; stale runtime evidence should be visible as a status/attention state, not a silent broken webview. | `RuntimeDetailView` now shows a broker-backed runtime status strip for brokered gateways, with Shared Browser room/current URL/last activity chips when available. The runtime toolbar exposes `Status`, `Reconnect`, `Reload`, `Open`, and local `Close View` controls. `WorkbenchModel.refreshRuntimeStatus(...)` updates runtime status, Shared Browser metadata, and Session Center records from the same broker `runtime_status` source. |
