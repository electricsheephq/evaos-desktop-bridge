...
| Issue | Acceptance | Implemented |
| --- | --- | --- |
| `#97` Shared Browser first-class Workbench gateway | Shared Browser exposes broker runtime status and user-facing open/reconnect/reload/status controls without raw proxy errors; stale runtime evidence should be visible as a status/attention state, not a silent broken webview. | `RuntimeDetailView` now shows a broker-backed runtime status strip for brokered gateways, with Shared Browser room/current URL/last activity chips when available. The runtime toolbar exposes `Status`, `Reconnect`, `Reload`, `Open`, and local `Close View` controls. When broker evidence says Shared Browser is inactive/unavailable, Workbench labels the brokered launch as `Start / Attach`, tells the user startup can take up to a minute after idle, and Session Center points to the same start-or-reattach action instead of surfacing raw proxy failure copy. `WorkbenchModel.refreshRuntimeStatus(...)` updates runtime status, Shared Browser metadata, and Session Center records from the same broker `runtime_status` source. |
