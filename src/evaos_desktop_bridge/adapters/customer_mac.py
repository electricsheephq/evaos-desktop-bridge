...
DISABLED_IPHONE_ACTIONS = {"scroll", "swipe"}
SENSITIVE_APPS = {
    "1Password",
    "App Store",
    "Keychain Access",
...
""".strip()
        ...
        return CommandResult(
            ...
            data={
                ...
                "experimental_or_disabled": {
                    ...
                    "reason": "Scroll and swipe remain disabled until repeated live evidence proves reliability.",
                },
                "forbidden": [
    ...
    def iphone_mirroring_status(self) -> CommandResult:
        ...
        return CommandResult(
            ...
            data={
                ...
                "disabled_actions": sorted(DISABLED_IPHONE_ACTIONS),
                "safety": {
    ...
    def iphone_mirroring_action(self, *, action: str, text: str | None = None, app_name: str | None = None, target_label: str | None = None, dry_run: bool = False) -> CommandResult:
        if action in DISABLED_IPHONE_ACTIONS:
            return CommandResult(
                ok=False,
                data={"performed": False, "would_perform": dry_run, "action": action},
                errors=[make_error(code="iphone_action_disabled_pending_evidence", message="This iPhone Mirroring action is disabled until repeated live evidence proves reliability.", guidance="Use supported named actions only; do not fall back to generic coordinates.")],
            )
    ...
    def _iphone_keyboard_action(self, action: str, key_code: str) -> CommandResult:
        result = self.runner(["osascript", "-e", f'tell application "System Events" to key code {key_code} using command down'], 5.0)
        if result.returncode != 0:
        ...
        return CommandResult(ok=True, data={"performed": True, "action": action}, provenance={"source": "iphone_mirroring"})

    def _press_iphone_target(self, *, target_label: str) -> CommandResult:
        if self.accessibility_checker() is False:
            return CommandResult(ok=False, data={"performed": False, "action": "tap_named_target", "target_label": target_label}, errors=[self._permission_error("accessibility", "tap a named iPhone Mirroring target")])
        pid = self._pid_for_app("iPhone Mirroring")
        if pid is None:
            return CommandResult(ok=False, data={"performed": False, "action": "tap_named_target", "target_label": target_label}, errors=[make_error(code="iphone_mirroring_not_running", message="iPhone Mirroring is not currently running.", guidance="Open iPhone Mirroring and rerun the named action.")])
        ...
        if result.returncode != 0:
            return CommandResult(ok=False, data={"performed": False, "action": "tap_named_target", "target_label": target_label}, errors=[make_error(code="iphone_target_lookup_unavailable", message="Unable to inspect iPhone Mirroring targets.", guidance=ACCESSIBILITY_GUIDANCE, permission="accessibility")], warnings=warnings)
        try:
            payload = json.loads(result.stdout.strip() or "{}")
        except json.JSONDecodeError:
            return CommandResult(ok=False, data={"performed": False, "action": "tap_named_target", "target_label": target_label}, errors=[make_error(code="iphone_target_lookup_parse_failed", message="Unable to parse iPhone Mirroring target lookup output.", guidance="Check pyobjc GUI dependencies in the bridge environment.")], warnings=warnings)
        if not payload.get("ok"):
            code = str(payload.get("error") or "iphone_target_press_failed").split(":", 1)[0]
            return CommandResult(ok=False, data={"performed": False, "action": "tap_named_target", "target_label": target_label, "matches": [self._safe_node(row) for row in payload.get("matches", [])]}, errors=[make_error(code=code, message="The named target could not be pressed safely.", guidance="Use exact visible labels only; do not fall back to generic coordinates.")], warnings=warnings)
        return CommandResult(ok=True, data={"performed": True, "action": "tap_named_target", "target_label": target_label, "matches": [self._safe_node(row) for row in payload.get("matches", [])]}, warnings=warnings, provenance={"source": "iphone_mirroring_ax"})
