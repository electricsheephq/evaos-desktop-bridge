...
async function providerProfilesPayload() {
    ...
    return redactConnectorSecrets({
        ...
        data: {
            ...
            active_provider_key: process.env.EVAOS_ACTIVE_PROVIDER_KEY ||
...
function customerMacControlEndpoint() {
...
function loadDesktopBridgeEnvFile() {
    if (process.env.EVAOS_DESKTOP_BRIDGE_URL && process.env.EVAOS_DESKTOP_BRIDGE_TOKEN)
        return;
...
async function cacheProviderGrantFromBroker(agentRuntime, providerKey, payload) {
    if (!isRecord(payload) || !isRecord(payload.agent_grant))
        return false;
...
async function runLegacyEnrollmentBridge(params, enrollmentCode) {
...
async function claimEnrollmentFromBroker(params, enrollmentCode) {
            ...
            errors: [
                {
                    ...
                    guidance: "Set EVAOS_CUSTOMER_MAC_CONTROL_URL or EVAOS_PROVIDER_DISCOVERY_URL, EVAOS_CUSTOMER_ID, and EVAOS_PROVIDER_AUTH_PROOF_SECRET on the VM.",
                },
            ],
        };
    }
